...
describe('Firestore Security Rules', () => {
  ...
  it('❌ Refuse modification householdId', async () => {
    ...
    await firebase.assertFails(
      testDoc.update({ householdId: 'house2' })
    );
  });
  ...
  it('✅ Permet lecture charges du foyer', async () => {
    ...
    await adminDb
      .collection('households').doc('house1')
      .collection('charges_fixes').doc('charge1')
      .set({
        nom: 'Électricité',
        montantMensuel: 50,
        payeur: 'user1'
  ...
  it('❌ Refuse accès charges autre foyer', async () => {
    ...
    await adminDb
      .collection('households').doc('house2')
      .collection('charges_fixes').doc('charge1')
      .set({
        nom: 'Électricité',
        montantMensuel: 50,
  ...
  it('✅ Permet création d\'une charge variable valide', async () => {
    const db = getFirestore({ uid: 'user1' });
    const adminDb = getAdminFirestore();
    await adminDb.collection('users').doc('user1').set({ householdId: 'house1' });

    const testRef = db.collection('households').doc('house1').collection('charges_variables').doc('chargeV1');
    
    await firebase.assertSucceeds(testRef.set({
  ...
  it('❌ Refuse création charge variable si montant est négatif ou zéro', async () => {
    ...
    const testRef = db.collection('households').doc('house1').collection('charges_variables').doc('chargeV2');
    
    await firebase.assertFails(testRef.set({
  ...
  it('❌ Refuse création charge variable si un champ obligatoire manque', async () => {
    ...
    const testRef = db.collection('households').doc('house1').collection('charges_variables').doc('chargeV3');
  ...
  it('❌ Refuse suppression d\'un compte mensuel finalisé', async () => {
    ...
    const adminDb = getAdminFirestore();
    await adminDb.collection('users').doc('user1').set({ householdId: 'house1' });
    await adminDb.collection('households').doc('house1').collection('comptes_mensuels').doc('octobre2023').set({
      moisAnnee: '10-2023',
      loyerTotal: 800,
      apportsAPL: 150,
      statut: 'finalisé'
    ...
    const testRef = db.collection('households').doc('house1').collection('comptes_mensuels').doc('octobre2023');
  ...
  it('❌ Refuse suppression d\'une catégorie par défaut', async () => {
    ...
    await adminDb.collection('households').doc('house1').collection('categories').doc('cat_alim').set({
      label: 'Alimentation',
      icon: 'food',
      isDefault: true
    });

    const testRef = db.collection('households').doc('house1').collection('categories').doc('cat_alim');
    await firebase.assertFails(testRef.delete());
  });
});
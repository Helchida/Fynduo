...
const ChargesFixesScreen: React.FC = () => {
  const { chargesFixes, isLoadingComptes, updateChargeFixe, deleteChargeFixe, currentMonthData, addChargeFixe, updateChargeFixePayeur} = useComptes();

  const { user } = useAuth();
  const [nom, setNom] = useState('');
  const [montant, setMontant] = useState('');
  const [payeur, setPayeur] = useState<string | null>(user?.id || null);
  ...
  const handleChargeUpdate = useCallback(async (id: string, newAmount: number) => {
    ...
    const handleChargeUpdatePayeur = useCallback(async (
        chargeId: string, 
        newPayeurUid: string, 
    ...
    const handleAddDepense = useCallback(async () => {
            ...
            if (!payeur) {
                Alert.alert("Erreur de saisie", "Veuillez sélectionner qui paie cette charge.");
                return;
            }

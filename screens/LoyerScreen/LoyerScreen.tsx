...
const LoyerScreen: React.FC = () => {
        ...
        const loadUsers = async () => {
            if (user?.householdId) {
                try {
                    ...
                    Alert.alert("Erreur", "Impossible de charger les utilisateurs du foyer.");
                } finally {
    ...
    const handleSave = async () => {
        ...
        for (const uid in apportsAPL) {
            ...
            const userDisplay = householdUsers.find(u => u.id === uid)?.displayName || 'un utilisateur';
            
            if (isNaN(aplValue) || aplValue < 0) {
                Alert.alert("Erreur", `Le montant APL pour ${userDisplay} est invalide.`);
                return;
            }
    ...
    return (
        <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
            ...
            <View style={styles.inputGroup}>
                ...
                <TouchableOpacity
                    style={[styles.input, styles.dropdownInput, isDisabled && styles.disabledInput]}
